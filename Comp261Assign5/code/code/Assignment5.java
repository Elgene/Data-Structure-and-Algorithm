...
public class Assignment5 {
	...
	private static final String[] ALGORITHMS = { "Huffman coding", "Lempel Ziv","Ngrams" };

	private JFrame frame;
	private JFileChooser fileChooser;

	// editor components.
	private JCheckBox buttonKPM;
	private JCheckBox bruteForceButton;
	private JCheckBox boyerMooreButton;
	...
	private void onAlgorithmRun() {
		try {
			...
			else if (algorithm.equals("Ngrams")) {
				// read in the file.
				String text = readFile(compressionInputFile);
				// run the algorithms.
				Ngrams ngr = new Ngrams();
				List<String> ngramList = new ArrayList<String>();

				for (int i = 0; i <= 5 ; i++) {
					ngramList.add(ngr.ngramsProbCal(text,i));
				}
				String output = ngr.getInformation();
				List<String> lines = new ArrayList<String>(ngramList);
				Files.write(compressionOutputFile.toPath(), (List<String>)lines, CHARSET);
				compressionOutput.append(output);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void initialise() {
		...
		buttonKPM = new JCheckBox("KMP ");
		bruteForceButton = new JCheckBox("Brute Force ");
		boyerMooreButton = new JCheckBox("Boyer Moore ");
		...
		searchField.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(buttonKPM.isSelected()) {
					double start = System.nanoTime();
					index = new KMP(pattern, text).search(pattern, text);
					JOptionPane.showMessageDialog(frame, "KMP :" + (System.nanoTime() - start) + "ns ");
					if (index == -1) {
				...
				if(bruteForceButton.isSelected()) {
					double start = System.nanoTime();
					index = new BruteForce().search(pattern, text);
					JOptionPane.showMessageDialog(frame, "Brute Force : " + (System.nanoTime() - start) + "ns ");
					if (index == -1) {
						JOptionPane.showMessageDialog(frame, "Pattern not found.");
					} else {
						textEditor.requestFocus();
				 ...
				 if(boyerMooreButton.isSelected()) {
					 int[] charTable = BoyerMoore.boyerCharacterTable(pattern);
					 int[] jumpTable = BoyerMoore.BoyerJumpTable(pattern);
					 int index = 0;
					 long start = System.nanoTime();
					 index =  BoyerMoore.boyerSearch(pattern, text, charTable, jumpTable);
					 JOptionPane.showMessageDialog(frame, "Boyer Moore : " + (System.nanoTime() - start) + "ns ");
		...
		controls.add(buttonKPM);
		controls.add(bruteForceButton);
		controls.add(boyerMooreButton);
